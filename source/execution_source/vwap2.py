...
def algo_loop_limit(trading_day, order_side, original_order_quantity, vwap_coefficients, tick_coef, limit):
    log_message('Beginning VWAP run: {:s} {:d} shares'.format(order_side, original_order_quantity))
    ...
    for index, row in trading_day.iterrows():
        ...
        # TRADING LOGIC
        # check where our FV is versus the BBO and constrain
        if order_side == 'b' and limit >= live_order_price:
            if fair_value >= ask_price and quantity_behind > round_lot:
                total_agg_count += 1
        ...
        elif order_side == 's' and limit <= live_order_price:
            if fair_value <= bid_price and quantity_behind > round_lot and quantity_remaining != 0:
                total_agg_count += 1
